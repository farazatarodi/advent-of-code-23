...
func Puzzle2(lines []string) {
	...
	for mainIndex := 0; mainIndex < len(hands); mainIndex++ {
		...
		handType := getHandType2(mainHand[0])
		...
		for i := 0; i < mainIndex; i++ {
			...
			if mainTypeNumber == typeNumber {
				for j := 0; j < len(hands[i][0]); j++ {
					...
					mainCharInt := convertToInt2(mainChar)
					targetCharInt := convertToInt2(targetChar)
	...
	fmt.Println("Day 7, Puzzle 1:", hands, sum)
}

func getHandType2(hand string) string {
...
func convertToInt2(card string) int {
